...
pub use crate::window_rule::{
    FloatingPosition, Match, PopupsRule, RelativeTo, ResolvedPopupsRules, WindowRule,
};
...
pub struct Config {
    ...
    pub adaptive_animation_profile: AdaptiveAnimationProfile,
    pub adaptive_animation_profile_override: Option<String>,
    pub action_palette: ActionPalette,
    pub mode_hud: ModeHud,
    pub niri_liquid: NiriLiquid,
    pub safe_mode: SafeMode,
    pub dispatch_aliases: Vec<DispatchAlias>,
    pub overlays: Overlays,
    pub unified_rules: Vec<UnifiedRule>,
    pub material_graphs: Vec<MaterialGraph>,
    pub animation_graphs: Vec<AnimationGraphDoc>,
    pub script: ScriptConfig,
    pub performance_budget: PerformanceBudget,
}

impl Config {
    pub fn resolved_animation_profile_name(&self) -> Option<&str> {
        self.adaptive_animation_profile_override
            .as_deref()
            .or(self.active_animation_profile.as_deref())
    }

    pub fn is_adaptive_animation_profile_active(&self) -> bool {
        self.adaptive_animation_profile_override.is_some()
    }

    pub fn resolved_animation_profile_label(&self) -> String {
        let base = self.resolved_animation_profile_name().unwrap_or("default");
        if self.is_adaptive_animation_profile_active() {
            format!("{base} auto")
        } else {
            base.to_string()
        }
    ...
    pub fn resolved_animation_profile(&self) -> Option<&AnimationProfile> {
        let profile_name = self.resolved_animation_profile_name()?;
    ...
    fn decode_children(
        ...
        for node in nodes {
            ...
            match name {
                ...
                "debug" => m_merge!(debug),
                "action-palette" => m_merge!(action_palette),
                "mode-hud" => m_merge!(mode_hud),
                "adaptive-animation-profile" => m_merge!(adaptive_animation_profile),
                ...
                "landmark" => m_push!(landmarks),
                "magnet" => m_merge!(magnet),
                "effect-preset" => m_push!(effect_presets),
                "animation-profile" => m_push!(animation_profiles),
                "animation-preset" => m_push!(animation_presets),
                ...
                "active-animation-profile" => {
                    ...
                    let val = iter
                        .next()
                        .ok_or_else(|| DecodeError::missing(node, "argument is required"))?;
                ...
                // Liquid-specific sections.
                "niri-liquid" => m_merge!(niri_liquid),
                "safe-mode" => m_merge!(safe_mode),
                "overlays" => m_merge!(overlays),
                "rule" => {
                    let doc = UnifiedRuleDoc::decode_node(node, ctx)?;
                    config.borrow_mut().unified_rules.push(doc.into_rule());
                }
                "material-graph" => {
                ...
                "script" => m_merge!(script),
                "performance-budget" => m_merge!(performance_budget),
                "animation-graph" => {
                    let doc = AnimationGraphDoc::decode_node(node, ctx)?;
                    config.borrow_mut().animation_graphs.push(doc);
                }
                "dispatch-alias" => {
                    let doc = DispatchAliasDoc::decode_node(node, ctx)?;
                    config.borrow_mut().dispatch_aliases.push(doc.into());
                }
    ...
    pub fn parse(path: &Path, text: &str) -> ConfigParseResult<Self, ConfigIncludeError> {
        ...
        let config = part
            .map(|_| {
                ...
                if config.action_palette.enable {
                    if let Some(ref bind_str) = config.action_palette.bind {
                        use std::str::FromStr;
                        if let Ok(key) = Key::from_str(bind_str) {
                            let new_bind = Bind {
                                key,
                                action: Action::ToggleActionPalette,
                                repeat: false,
                                cooldown: None,
...
mod tests {
    ...
    fn parse() {
        ...
        Config {
            ...
            window_rules: [
                WindowRule {
                    ...
                    background_effect: BackgroundEffectRule {
                        ...
                        foreground_liquid: None,
                        foreground_refraction: None,
                        foreground_chromatic_aberration: None,
                        bloom: None,
                    },
                    effect_preset: None,
                    popups: PopupsRule {
            ...
            active_animation_profile: None,
            adaptive_animation_profile: AdaptiveAnimationProfile {
                enable: false,
                active: None,
                idle: None,
                on_battery: None,
            ...
            adaptive_animation_profile_override: None,
            ...
            dispatch_aliases: [],
            overlays: Overlays {
                default_material: "dashboard-glass",
